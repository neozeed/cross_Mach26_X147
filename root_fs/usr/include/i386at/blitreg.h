 ...
 * HISTORY
 * $Log:	blitreg.h,v $
 * Revision 1.1  91/05/09  13:40:56  jaap
 ...
 * CMU-HISTORY
 * $CMU-Log:	blitreg.h,v $
 * Revision 1.4.1.4  90/11/27  13:41:03  rvb
...
struct  blitdev {
	volatile u_char graphmem[BLIT_MEMSIZE];
	volatile u_char reg786[BLIT_REGSIZE];
	volatile u_char pad1[BLIT_REGPAD];
	volatile u_char sram[BLIT_SRAMSIZE];
	volatile u_char pad2[BLIT_SRAMPAD];
	volatile u_char eprom[BLIT_EPROMSIZE];
};
...
#define WRITEREG16(base,offset,val) \
	(*((volatile u_short *)((base)->reg786+(offset)))) = (val)

#define READREG(base,offset) \
	(*((volatile u_short *)(((base)->reg786+(offset)))))

#define WRITEROM(romp,offset,val) \
	(*((volatile u_short *)((romp)+(offset)))) = (val)

#define READROM(romp,offset) \
	(*((volatile u_short *)(((romp)+(offset)))))
...
typedef struct {
	volatile u_short vidstat;	/* video status */
	volatile u_short intrmask;	/* interrupt mask */
	volatile u_short trip_point;
	volatile u_short frame_intr;	/* frame interrupt */
	volatile u_short reserved1;
	volatile u_short crtmode;	/* CRT controller mode */
	volatile u_short hsyncstop;	/* monitor parameters */
	volatile u_short hfldstart;
	volatile u_short hfldstop;
	volatile u_short linelength;
	volatile u_short vsyncstop;
	volatile u_short vfldstart;
	volatile u_short vfldstop;
	volatile u_short vframelen;
	volatile u_short descl;		/* descriptor pointer low part */
	volatile u_short desch;		/* descriptor pointer high part */
	volatile u_short reserved2;
	volatile u_short xyzoom;
	volatile u_short fldcolor;
	volatile u_short bordercolor;
	volatile u_short bpp_pad1;
	volatile u_short bpp_pad2;
	volatile u_short bpp_pad4;
	volatile u_short csrmode;	/* & CsrPad */
	volatile u_short cursorx;	/* cursor x location */
	volatile u_short cursory;	/* cursor y location */
	volatile u_short cursorpat[16];	/* cursor pattern */
} DPCONTROLBLK;
...
typedef struct {
	volatile u_short lines;		/* (lines in strip) - 1 */
	volatile u_short linkl;		/* link to next strip low part */
	volatile u_short linkh;		/* link to next strip high part */
	volatile u_short tiles;		/* C bit, (tiles in strip) - 1 */
} STRIPHEADER;
...
typedef struct {
	volatile u_short bitmapw;	/* width of bitmap */
	volatile u_short meml;		/* btb mem address low part */
	volatile u_short memh;		/* btb mem address high part */
	volatile u_short bppss;		/* bpp, start and stop fields */
	volatile u_short fetchcnt;	/* fetch count */
	volatile u_short flags;		/* various flags */
} TILEDESC;
