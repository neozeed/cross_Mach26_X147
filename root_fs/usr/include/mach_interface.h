...
mig_external kern_return_t pager_cache
#if	defined(LINTLIBRARY)
    (memory_control, should_cache)
	memory_object_control_t memory_control;
	boolean_t should_cache;
{ return pager_cache(memory_control, should_cache); }
#else
#if	(defined(__STDC__) || defined(c_plusplus))
(
	memory_object_control_t memory_control,
...
{ return host_info(target_task, info); }
#else
#if	(defined(__STDC__) || defined(c_plusplus))
(
	port_t target_task,
...
/* Routine slot_info */
mig_external kern_return_t slot_info
#if	defined(LINTLIBRARY)
    (target_task, slot, info)
	task_t target_task;
	int slot;
	machine_slot_data_t *info;
{ return slot_info(target_task, slot, info); }
#else
#if	(defined(__STDC__) || defined(c_plusplus))
(
	task_t target_task,
...
/* Routine cpu_control */
mig_external kern_return_t cpu_control
#if	defined(LINTLIBRARY)
    (target_task, cpu, running)
	task_t target_task;
	int cpu;
	boolean_t running;
{ return cpu_control(target_task, cpu, running); }
#else
#if	(defined(__STDC__) || defined(c_plusplus))
(
	task_t target_task,
...
mig_external kern_return_t pager_data_provided_inline
#if	defined(LINTLIBRARY)
    (memory_control, offset, data, lock_value)
	memory_object_control_t memory_control;
	vm_offset_t offset;
	vm_page_data_t data;
	vm_prot_t lock_value;
{ return pager_data_provided_inline(memory_control, offset, data, lock_value); }
#else
#if	(defined(__STDC__) || defined(c_plusplus))
(
	memory_object_control_t memory_control,
